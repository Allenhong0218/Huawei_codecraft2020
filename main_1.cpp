...
#define MAXCHARSIZE7 9000000000
...
void initGraph(bool initGinv){
    ...
    nodeCnt=tmp.size();
    charidsComma = vector<char[15]>(nodeCnt);
    auto tmpcnt=0;
    for(uint &x:tmp){
...
inline void get3neighbour(Node &vi,vector<char> &neighbour1){
    auto &Gi=Ginv[vi.to];
    if(!Gi.empty()){
        auto vj =lower_bound(Gi.begin(),Gi.end(),vi);
        for(;vj!=Gi.end();++vj){
            auto mj = vj->money;
            neighbour1[vj->to] |= 0x1;
            auto &Gj = Ginv[vj->to];
            if(!Gj.empty()){
...
void balancedCirChunk(uint chunkID,uint idxBegin,uint idxEnd){
    ...
    for(uint i=idxBegin;i<idxEnd;i+=4){
        ...
        if(!G[i].empty()){//i->j->k->l->m->n->o->i
            ...
            *tmp_path_it++ = i;
            visited[i]=true;
            vector<char >neighbour(nodeCnt,0);
                                ...
                                if(!visited[vl->to] && check(mk,vl->money)&& !G[vl->to].empty() ){
                                    *tmp_path_it++ = vl->to;//i,j,k,l,path长为4
                                    ...
                                    for(;vm!=G[vl->to].end();++vm){//
                                        if( neighbour[vm->to] & FAST3 && !visited[vm->to] && check(vl->money,vm->money)&& !G[vm->to].empty()){
                                            *tmp_path_it++ = vm->to;//i,j,k,l,m,path长度为5
                                            visited[vm->to] = true;
                                            auto vn = lower_bound(G[vm->to].begin(),G[vm->to].end(),headNode);
                                            if( neighbour[vm->to] & FAST1 && 
                                            ...
                                            for(;vn!=G[vm->to].end();++vn){//
                                                if(neighbour[vn->to] & FAST2 && !visited[vn->to] && check(vm->money,vn->money)&& !G[vn->to].empty()){
                                                    *tmp_path_it++ = vn->to;//i,j,k,l,m,n,path长度为6
                                                    visited[vn->to] = true;
                                                    auto vo =  lower_bound(G[vn->to].begin(),G[vn->to].end(),headNode);
                                                    if( neighbour[vn->to] & FAST1 && 
                                                    ...
                                                    for(;vo!=G[vn->to].end();++vo){
                                                        if(neighbour[vo->to] & FAST1 && !visited[vo->to]  && check(vn->money,vo->money)&& !G[vo->to].empty()){
