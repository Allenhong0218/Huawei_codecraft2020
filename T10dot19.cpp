...
vector<vector<Node>::iterator> lb_it[MAXTHREADNUM];
...
void initGraph(bool initGinv){
    ...
    lb_it[0]=vector<vector<Node>::iterator>(nodeCnt);
    lb_it[1]=vector<vector<Node>::iterator>(nodeCnt);
    lb_it[2]=vector<vector<Node>::iterator>(nodeCnt);
    lb_it[3]=vector<vector<Node>::iterator>(nodeCnt);
            ...
            for(;vj!=G[i].end();++vj){
                if(!visited[vj->to] && !G[vj->to].empty()){
                    *tmp_path_it++=vj->to;//i,j
                    visited[vj->to]=true;
                    auto mj = vj->money;
                    auto vk = lower_bound(lb_it[chunkID][vj->to],G[vj->to].end(),headNode);
                    lb_it[chunkID][vj->to] = vk;
                    for(;vk!=G[vj->to].end();++vk){
                        auto mk = vk->money;
                        if(!visited[vk->to] && check(mj,mk)&& !G[vk->to].empty()){
                            *tmp_path_it++ = vk->to;//i,j,k
                            visited[vk->to]=true;
                            auto vl = lower_bound(lb_it[chunkID][vk->to],G[vk->to].end(),headNode);
                            lb_it[chunkID][vk->to] = vl;
                            if( neighbour1[vk->to] && 
                                check(mk,vl->money) &&
                                ...
                                if(!visited[vl->to] && check(mk,vl->money)&& !G[vl->to].empty() ){
                                    *tmp_path_it++ = vl->to;//i,j,k,l,path长为4
                                    ...
                                    for(;vm!=G[vl->to].end();++vm){//
                                        if( neighbour3[vm->to] && !visited[vm->to] && check(vl->money,vm->money)&& !G[vm->to].empty()){
                                                ...
                                                if(neighbour2[vn->to] && !visited[vn->to] && check(vm->money,vn->money)&& !G[vn->to].empty()){
                                                    *tmp_path_it++ = vn->to;//i,j,k,l,m,n,path长度为6
                                                    ...
                                                    for(;vo!=G[vn->to].end();++vo){
                                                        if(neighbour1[vo->to] && !visited[vo->to]  && check(vn->money,vo->money)&& !G[vo->to].empty()){
