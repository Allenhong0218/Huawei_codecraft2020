...
#ifdef TEST
    // PRINT<< cnt_remove <<" Nodes eliminated in TopoSort"<<endl;
#endif
}

inline void get3neighbour(Node &vi,vector<bool> &unset,vector<bool> &neighbour1){
    auto &Gi=Ginv[vi.to];
    if(!Gi.empty()){
...
void step3dfs(uint headmoney,uint premoney,vector<bool> &visited,vector<bool> &neighbour,
              vector<bool> &neighbour1,uint chunkID,vector<Path> &chunkPath,Node &head,uint cur,uint depth,vector<uint> &path){
    ...
    //     // PRINT<<cur;
    //     PRINT<<endl;
    //     PRINT<<"Find 977: "<<premoney<<" "<<it->money<<" "<<check(premoney,it->money)<<endl;
    //     PRINT<<"Find 977: "<<cur<<"->"<<it->to<<endl;
    //     for(;it!=Curend;++it){
    //         PRINT<<it->to<<",";
    //     }
    //     PRINT<<endl;
    ...
    if(depth<7){//DFS深度小于7
        for(;it!=Curend;++it){
            if( neighbour[it->to] && !visited[it->to] &&check(premoney,it->money)){
                step3dfs(headmoney,it->money,visited,neighbour,neighbour1,chunkID,chunkPath,head,it->to,depth+1,path);
            }
        }
    }
    // }
...
// #ifdef TEST
//         if(i%100==0)
//             PRINT<<chunkID<<": "<<(float)((i-idxBegin))/float((idxEnd-idxBegin))*100.0<<"%"<<endl;
// #endif
        // auto &Ni =G[i];
        if(!G[i].empty()){//i->j->k->l->m->n->o->i
            ...
            // bool neighbour[nodeCnt],neighbour1[nodeCnt];
            auto headNode = Node(i,0);
            get3neighbour(headNode,neighbour,neighbour1);
                        ...
                        if(!visited[vk->to] && !G[vk->to].empty() && check(mj,mk)){
                            ...
                            for(;vl!=G[vk->to].end();++vl){//
                                if(!visited[vl->to] && !G[vl->to].empty() && check(mk,vl->money)){
                                    ...
                                    //     bool find977=false;
                                    // #ifdef TEST
                                    //     if(vk->to==61243&&vl->to==31991 && vm->to==22786){
                                    //         PRINT<<"Found!!!"<<endl;
                                    //         PRINT<<i<<"->"<<vj->to<<"->"<<vk->to<<"->"<<vl->to<<"->"<<vm->to<<":money:"<<vm->money<<endl;
                                    //         PRINT<<vm->money*5ll<<endl;
                                    //         PRINT<<vl->money*3ll<<endl;
                                    //         PRINT<<vl->money<<" "<<vm->money<<" Check money: "<<check(vl->money,vm->money)<<endl;
                                    //         find977=true;
                                    //     }
                                    // #endif
// #ifdef TEST
//                                             bool flag45457 =false;
//                                             if(vm->to==45457 && headNode.to==0){
//                                                 PRINT<<"Found"<<endl;
//                                                 PRINT<<vj->to<<"->"<<vk->to<<"->"<<vl->to<<"->"<<vm->to<<"->"<<endl;
//                                                 PRINT<<vj->money<<"->"<<vk->money<<"->"<<vl->money<<"->"<<vm->money<<"->"<<endl;
//                                                 flag45457 = true;
//                                             }
// #endif
                                            step3dfs(mj,vm->money,visited,neighbour,neighbour1,chunkID,chunkAns[chunkID],headNode,vm->to,5,path);
                                        }
                                    }
                                    path.pop_back();
                                    visited[vl->to]=false;
            ...
            path.pop_back();
            visited[i]=false;
            // step3dfs(neighbour,chunkID,chunkAns[chunkID],i,i,1,path);
        }
    }
#ifdef  TEST
// PRINT <<endl;
...
// void saveCharmmap(string &outputFile){
// #ifdef TEST
//     PRINT<<"Mmap save : "<<endl;
//     auto start = system_clock::now();
// #endif
//     uint totalSum=0;
//     char *ansChar_end[] = {ansChar[0].mmaptest,ansChar[1].mmaptest,ansChar[2].mmaptest,ansChar[3].mmaptest,ansChar[4].mmaptest,ansChar[5].mmaptest,ansChar[6].mmaptest,ansChar[7].mmaptest};
//     for(auto &chunkans:chunkAns){
//         totalSum+=chunkans.size();
//         for(auto &tmpPath:chunkans){
//             uint tmpsize = tmpPath.size();            
//             writetochar(ansChar[tmpsize].length,ansChar_end[tmpsize],tmpPath);
//         }
//     }
//     uint fd;
//     char *addr;
...
//     PRINT<<"Total loop: "<<totalSum<<endl;
// #endif
//     char *bufferlen=ansChar[0].mmaptest;
//     unsigned_to_decimal(ansChar[0].length,totalSum,bufferlen);
//     ansChar[0].mmaptest[ansChar[0].length-1]='\n';
//     uint totallen=ansChar[0].length;
//     for(uint i=3;i<8;++i){
//         totallen+=ansChar[i].length;
//     }
//     lseek(fd,totallen-1,SEEK_SET);
//     write(fd,"\n",1);
//     addr = (char *)mmap(NULL, totallen-1, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
//     memcpy(addr,ansChar[0].mmaptest ,ansChar[0].length);
//     uint tmplen=ansChar[0].length;
//     for(uint i=3;i<7;++i){
//         memcpy(addr+tmplen,ansChar[i].mmaptest,ansChar[i].length);
//         tmplen += ansChar[i].length;
//     }
//     memcpy(addr+tmplen,ansChar[7].mmaptest,ansChar[7].length-1);
//     close(fd);
...
#endif
    // savemmap(answerFile);
    // saveCharmmap(answerFile);
    saveFwrite(answerFile);
    // saveStrMmap(answerFile);
    exit(0);
#ifdef TEST
end  = system_clock::now();
duration= duration_cast<microseconds>(end - start);
    PRINT << "Mmap save :" << float(duration.count()) * microseconds::period::num / microseconds::period::den << "s" << endl;
#endif
    return 0;
}
